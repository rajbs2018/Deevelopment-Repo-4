...
void man(char *name)
{
	...
	{
		printf("Descrption : Used to write into regular file\n");
	...
	{
		printf("Description : Used to open exixting file\n");
		printf("Usage : open File_name mode\n");
	}
	else if((name,"close")==0)
	{
		printf("Description : Used to close opened file\n");
		printf("Usage : close File_name\n");
	}
	else if((name,"closeall")==0)
	{
...
{
	if((UFDTArr[fd].ptrfiletable->mode)!=WRITE)&&((UFDTArr[fd].ptrfiletable)!=READ + WRITE)) return -1;
	
	if(((UFDTArr[fd].ptrfiletable->ptrinode->permission)!= WRITE)&&((UFDTArr[fd].ptrfiletabe->ptrinode->permission)!= READ + WRITE))		return-1;
...
int LseekFile(int fd,int size,int from)
{
	...
	if((UFDTArr[fd].ptrfiletable->mode ==READ)||(UFDTArr[fd].ptrfiletable -> mode == READ + WRITE))
	{
		if(from == START)
		{
			if(((UFDTAArr[fd].ptrfiletable->readoffset)+size)>UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) 		reurn -1;
	...
	while(1)
	{
		...
		else if(count == 3)
		{
			if(strcmp(command[0],"create")== 0)
			{
				...
				if(ret >= 0)
					printf("File is succesfully create with file descriptor : %d\n",ret);
				if(ret == -1)
					printf("ERROR : Incorrect parameter\n");
				if(ret = -2)
				...
				if(ret == -4)
					printf("ERROR : Memory allocated failure\n");
				continue;
			}
